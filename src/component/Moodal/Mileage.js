...
function Mileage({ closeModal, modalChange, history, handlerChange, isChange }) {
  ...
  useEffect(() => {
    axios.get(`${process.env.REACT_APP_REST_API_SERVER_IP_PORT}/member`).then((response) => {
      setData({
  ...
  const handleCharge = () => {
    ...
    axios.post(`${process.env.REACT_APP_REST_API_SERVER_IP_PORT}/mileage`, {chargeAmount : chargeMileage, memberEmail : userEmail})
    .then((response) => {
